...
class Parent(SerializableMixin):
  """A class representing a parent node in a graph.
    ...
    Attributes:
        ...
        type (str): The type of the parent node.
    """
...
def _fetch_parents_uncached(endpoint, dcid: str) -> list[dict[str, str]]:
  """Fetches the immediate parents of a given DCID from the endpoint, without caching.

    This function performs a direct, uncached call to the underlying data source. It exists
...
def fetch_parents_lru(endpoint, dcid: str) -> tuple[Parent, ...]:
  ...
  return tuple(Parent(p["dcid"], p["name"], p["type"]) for p in parents)
...
def build_ancestry_map(
  ...
  """Constructs a complete ancestry map for the root node using parallel
    ...
    Args:
        ...
        max_workers (int): Max number of threads to use for parallel fetching.
